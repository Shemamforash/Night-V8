...
public class StarfishBehaviour : Boss
{
	...
	public static StarfishBehaviour Instance() => _instance;
	...
	private float GetCurrentAngle(int i)
	{
		...
		if (oldSinTime < 0f && _sinTime >= 0f || oldSinTime > 0f && _sinTime <= 0f)
		{
			_playAudio = true;
		}
		else
	...
	public static float GetRadiusModifier() => _radiusModifier;
	...
	public List<CanTakeDamage> GetSections() => new List<CanTakeDamage>(Sections);
}