...
namespace Game.Combat.Generation
{
	public static class WorldGrid
	{
		...
		public static void FinaliseGrid()
		{
			...
			{
				for (int y = 0; y < GridWidth; ++y) _searchGrid.SetWalkableAt(x, y, Grid[x][y].Reachable);
			});
		}
		...
		public static Cell GetCellNearMe(Vector2 position, float distanceMax, float distanceMin = 0) => GetCellNearMe(WorldToCellPosition(position), distanceMax, distanceMin);
		...
		public static Cell GetEdgeCell() => _edgePositionList.RandomElement();
		...
		private static void GenerateBaseGrid()
		{
			...
			for (int x = 0; x < GridWidth; ++x)
			{
				...
				for (int y = 0; y < GridWidth; ++y)
				{
					...
					Grid[x][y] = c;
					if (distanceSquared > outOfRangeDistanceSquared)
					{
						_outOfRangeList.Add(c);
					}
