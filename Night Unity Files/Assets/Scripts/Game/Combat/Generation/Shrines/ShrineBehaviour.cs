...
public abstract class ShrineBehaviour : BasicShrineBehaviour
{
	...
	private IEnumerator FailGlow()
	{
		...
		List<SpriteRenderer> dangerSprites   = DangerIndicator.transform.FindAllComponentsInChildren<SpriteRenderer>();
		while (glowTime > 0f)
		{
			if (CombatManager.Instance() == null) break;
			...
			dangerSprites.ForEach(s => s.color = c);
