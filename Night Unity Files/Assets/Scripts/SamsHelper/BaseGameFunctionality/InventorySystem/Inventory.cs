...
//            IncrementResource("Essence", 20);
		...
		private static List<ResourceItem> InventoryResources() => _resources.Values.ToList();
		...
		public static int GetResourceQuantity(string type) => GetResource(type)?.Quantity() ?? 0;
		...
		{
			string type = root.ParseString("Template");
