...
namespace TriangleNet.Tools
{
	public static class PolygonValidator
	{
		...
		private static double DotProduct(Point a, Point b, Point c) => (a.x - b.x) * (c.x - b.x) + (a.y - b.y) * (c.y - b.y);

		//  Returns the length of cross product AB x BC.
		private static double CrossProductLength(Point a, Point b, Point c) => (a.x - b.x) * (c.y - b.y) - (a.y - b.y) * (c.x - b.x);
