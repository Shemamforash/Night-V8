...
namespace Game.Global
{
	public class JournalEntry
	{
		...
		private string ConvertTitle(string title, int partNumber) => title + " " + ToRoman(partNumber);
		...
		{
			ReadJournals();
			string   journalString = root.ParseString("Journals");
			...
			StoryController.StorySeen = root.ParseBool("StorySeen");
		}
		...
		private static void LoadCharacterEntries()
		{
			...
			{
				string         title           = characterNode.ParseString("Title");
				string         characterString = characterNode.ParseString("Character");
				CharacterClass characterClass  = CharacterTemplate.StringToClass(characterString);
				int            partNumber      = characterNode.ParseInt("PartNumber");
				string         text            = characterNode.ParseString("Text");
		...
		private static void LoadWandererEntries()
		{
			...
			{
				string          title             = wandererNode.ParseString("Title");
				string          environmentString = wandererNode.ParseString("Environment");
				EnvironmentType environmentType   = Environment.StringToEnvironmentType(environmentString);
				int             partNumber        = wandererNode.ParseInt("PartNumber");
				string          text              = wandererNode.ParseString("Text");
		...
		private static void LoadNecromancerEntries()
		{
			...
			{
				string          title           = necromancerNode.ParseString("Title");
				EnvironmentType environmentType = Environment.StringToEnvironmentType(title);
				int             partNumber      = necromancerNode.ParseInt("PartNumber");
				string          text            = necromancerNode.ParseString("Text");
				JournalEntry    entry           = new JournalEntry(title, text, partNumber, (int) environmentType);
		...
		private static void LoadLoreEntries()
		{
			...
			{
				string          title             = loreNode.ParseString("Title");
				string          environmentString = loreNode.ParseString("Environment");
				EnvironmentType environmentType   = Environment.StringToEnvironmentType(environmentString);
				bool            unique            = loreNode.ParseBool("Unique");
				string          text              = loreNode.ParseString("Text");
