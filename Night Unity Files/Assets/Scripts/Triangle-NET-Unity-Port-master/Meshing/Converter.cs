...
namespace TriangleNet.Meshing
{
	...
	public static class Converter
	{
		...
		public static DcelMesh ToDCEL(Mesh mesh)
		{
			...
			foreach (var e in boundary.Values) e.next = boundary[e.twin.origin.id];
		...
		/// </summary>
		public static Mesh ToMesh(Polygon polygon, IList<ITriangle> triangles) => ToMesh(polygon, triangles.ToArray());
		...
		public static Mesh ToMesh(Polygon polygon, ITriangle[] triangles)
		{
			...
			for (i = 0; i < elements; i++) mesh.MakeTriangle(ref tri);
			...
			if (mesh.behavior.Poly)
			{
				...
				for (i = 0; i < segments; i++) mesh.MakeSegment(ref subseg);
			}
