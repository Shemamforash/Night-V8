...
namespace Facilitating.UIControllers
{
	public class UiArmourUpgradeController : UiInventoryMenuController
	{
		...
		public static void Load(XmlNode root) => _unlocked = root.ParseBool("Armour");
		...
		{
			_armourController = CharacterManager.SelectedCharacter.Armour;
		...
		{
			if (_armourController.CurrentLevel == ArmourController.MaxLevel)
			{
		...
		private void Upgrade()
		{
			...
			_upgradeMessage.SetText("Upgraded to " + _armourController.Name);
		...
		{
			_nameText.SetText(_armourController.Name);
